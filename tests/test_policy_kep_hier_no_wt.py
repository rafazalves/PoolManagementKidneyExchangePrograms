...
import kep.policies.policy_kep_hier_no_wt as policy_hier_no_wt
from kep.compatibility_graph import Digraph


def test_priority_compatibility_no_wt(load_kep_data):
    """
    ...
    All pairs have the same waiting time, except pair 2 which has been waiting longer. Since in this solver we do not consider waiting time, it should not affect the outcome.

    Expected Result:
    Cycle A or B should be chosen, because if all pairs have same compatibility and we ignore waiting time, both cycles have equal "normal" weight. 
    However, since we use a random weight tie-breaker, we expect one of the cycles to be chosen based on the random weight assigned.
    """
    ...
    # Execute the solver
    outputlist, selected_structures = policy_hier_no_wt.solver(digraph, data, max_cycle=3, max_chain=3, state=None)
    ...
    # Check if the nodes of the winning cycle are exactly {0, 2, 3} or {0, 1, 4} depending on random weight
    assert set(cycle) == {0, 2, 3} or set(cycle) == {0, 1, 4}, \
        f"Priority Error: The solver chose cycle {cycle} instead of cycle [0, 2, 3] or [0, 1, 4]."

    # Since the cycle [0, 2, 3] or [0, 1, 4] has 3 incompatible pairs, the total should be 3.
...
def test_priority_incompatible_vs_half(load_kep_data):
    ...
    Expected Result:
    Cycle A has a higher weight and should be chosen, because pair 2 is half-compatibility which makes Cycle B less favorable than Cycle A, which has all incompatible pairs.
...
def test_solver_empty_graph():
    ...
    results, structures = policy_hier_no_wt.solver(g, data, 2, 2, {})
...
def test_logs_and_reverse_execution(load_kep_data, monkeypatch, capsys):
    ...
    monkeypatch.setattr(policy_hier_no_wt, "LOG", True)
    monkeypatch.setattr(policy_hier_no_wt, "REVERSE", 1)

    with patch("kep.policies.policy_kep_hier_no_wt.gp.Model") as MockModel:
        ...
        policy_hier_no_wt.solver(digraph, data, max_cycle=3, max_chain=3, state=None)
