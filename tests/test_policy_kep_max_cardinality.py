...
import kep.policies.policy_kep_max_cardinality as policy_max_cardinality
from kep.compatibility_graph import Digraph


def test_priority_no_compatibility_no_wt(load_kep_data):
    """
    ...
    All pairs have COMPATIBILITY=1 (Incompatible), except pairs 1 and 4 which have COMPATIBILITY=2 (Half-Compatible).
    ...
    digraph, data = load_kep_data("pool3_test_solver.csv", "arc3_test_solver.txt")

    # Execute the solver
    outputlist, selected_structures = policy_max_cardinality.solver(digraph, data, max_cycle=3, max_chain=3, state=None)

    print(f"\n[DEBUG] Selected structures: {selected_structures}")

    assert len(outputlist) == 6, "The outputlist should have exactly 6 elements."
    assert outputlist[0] == 5, f"Should have 5 nodes, but got {outputlist[0]}."
    ...
    assert set(cycle) == {0, 1, 4}, \
        f"The solver chose cycle {cycle} instead of cycle [0, 1, 4]."

    # Since the cycle [0, 1, 4] has 1 incompatible pairs, the total should be 1.
    assert outputlist[2] == 1, f"Expected to count 1 incompatible pairs, but counted {outputlist[2]}."

def test_priority_incompatible_vs_half(load_kep_data):
    """  
    Test priority compatibility.
    
    Scenario:
    - Cycle A: 0 -> 4 -> 7 -> 0.
      Consists of pairs 0, 4, 7.
      
    - Cycle B: 0 -> 1 -> 2 -> 3 -> 0.
      Consists of pairs 0, 1, 2, 3.

    Since in this solver we do not consider waiting time nor compatibility, it should not affect the outcome.

    Expected Result:
    Cycle B has a higher weight (based on number of transplants (4)) and should be chosen.
    ...
    # Load the files for the test
    digraph, data = load_kep_data("pool4_test_solver.csv", "arc4_test_solver.txt")

    # Execute the solver
    outputlist, selected_structures = policy_max_cardinality.solver(digraph, data, max_cycle=4, max_chain=3, state=None)
    ...
    # Check if the nodes of the winning cycle are exactly {0, 1, 2, 3}
    assert set(cycle) == {0, 1, 2, 3}, \
        f"Priority Error: The solver chose cycle {cycle} instead of cycle [0, 1, 2, 3] (which has more transplants)."
    ...
    # Since the cycle [0, 1, 2, 3] has 2 incompatible pairs plus the pair 6 from the chain, the total should be 3.
...
def test_solver_empty_graph():
    ...
    results, structures = policy_max_cardinality.solver(g, data, 2, 2, {})
...
def test_logs_and_reverse_execution(load_kep_data, monkeypatch, capsys):
    ...
    monkeypatch.setattr(policy_max_cardinality, "LOG", True)
    monkeypatch.setattr(policy_max_cardinality, "REVERSE", 1)

    with patch("kep.policies.policy_kep_max_cardinality.gp.Model") as MockModel:
        ...
        policy_max_cardinality.solver(digraph, data, max_cycle=3, max_chain=3, state=None)
...
def test_gurobi_infeasible(load_kep_data, monkeypatch, capsys):
    '''
    Test Gurobi infeasibility handling.
    '''
    digraph, data = load_kep_data("pool2_test_solver.csv", "arc2_test_solver.txt")
    ...
    with patch("kep.policies.policy_kep_max_cardinality.gp.Model") as MockModel:
        ...
        mock_var = MagicMock()
        mock_var.X = 0.0
        mock_instance.addVar.return_value = mock_var

        policy_max_cardinality.solver(digraph, data, max_cycle=3, max_chain=3, state=None)

    captured = capsys.readouterr()

    assert "Solver finished with status" in captured.out
