...
def optimize_lexicographic_global(cycles, chains, cf_constraints, lexcoefs):
    """
    Lexicographically maximize global waiting times.
    Higher waiting times have higher priority (between same compatibility type).
    """
    ...
    model = gp.Model("Lexicographic_Global_WT")
    prepare_model(model)
...
def solver(digraph, data, max_cycle, max_chain, state):
    ...
    # Calculate waiting periods and compatibility scores for each pair
    waitperiods_pairs = [0 for _ in digraph.verts]

    for i in digraph.verts:
        # 1. Calculate the base Waiting Period
        base_score = ((data["epoch"] - 1) - int(characterization[i][COL_PAIR_ARRIVAL] / (data["tbm"] + 1)))
        ...
        waitperiods_pairs[i] = base_score
    ...
    lexcoefs = create_global_wtime_coefs(data["epoch"], cycles, chains, waitperiods_pairs, characterization)

    # 1. Solve Lexicographic waiting-time optimization
    if LOG:
        print ("\nRunning Global Lexicographic Waiting Time Optimization...")

    lex_sol, optimal_wt_profile, NumUnsolvedProbs = optimize_lexicographic_global(cycles, chains, cf_constraints, lexcoefs)
    ...
    for lvl, val in optimal_wt_profile.items():
        coef = lexcoefs[lvl]
        model.addConstr(
