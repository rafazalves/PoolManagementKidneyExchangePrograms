...
def solver(digraph, data, max_cycle, max_chain, state):
    """
    Static Solver: Maximizes only the number of transplants.
    """
    ...
    if LOG:
        print(f"Found {len(cycles)} cycles and {len(chains)} chains.")

    # Pre-calculate constraints
    cf_constraints = create_cf_constraints(digraph.nv, cycles, chains)

    # Optimization Model
    model = gp.Model("Static_Max_Transplants")
    prepare_model(model)
    ...
    for i, val in enumerate(finalsol):
        if val > 0.5:
            if x[i].VarName.startswith("c_"):
                ...
                # Stats de incompatibilidade (apenas contagem)
                for node in obj:
